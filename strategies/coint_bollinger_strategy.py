...
class CointegrationBollingerBandsStrategy(AbstractStrategy):
    ...
    Requires:
        ...
        weights - The weight vector describing a "unit" of the portfolio
        entry_z - The z-score trade entry threshold
        exit_z - The z-score trade exit threshold
        base_quantity - Number of "units" of the portfolio to be traded
    """
    ...
    def go_short_units(self):
        """
        Go short the appropriate number of "units" of the portfolio to open
        a new position or to close out a long position.
        """
        for i, ticker in enumerate(self.tickers):
            if self.weights[i] < 0.0:
    ...
    def calculate_signals(self, event):
        ...
        if event.type == EventType.BAR:
            ...
            if all(self.latest_prices > -1.0):
                # Calculate portfolio market value via dot product of ETF prices with portfolio weights
